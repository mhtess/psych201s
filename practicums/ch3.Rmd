...
# numSamples = number of samples for inference
runModel = function(model, data, numSamples = 10000) {
  # The following command calls WebPPL with specific options.
  wp <- webppl(
    program_code = model,
    data = observed_data,
    data_var = "observed_data",
    inference_opts = list(method="MCMC", samples = numSamples, burn = numSamples/10),
    ggmcmc = TRUE,
    model_var = "model",
    packages = c("./utils")
    )
