...
Try to solve the label-switching problem by using the dunif(0,mubon) approach instead of the logit transform.
...
var model = function() {
  // Assign groups with baserate phi
  var phi = beta(5,5) 
  var sampleGroup = function() { return flip(phi) ? "bonafide" : "cheater" }
  ...
  foreach(utils.range(0, n_people), function(person_id) {
    ...
    var successRate = (pAssignment == "bonafide")  ? 
                       beta_mu_lambda(muBon, lambdaBon) :
                       beta_mu_lambda(muCheat, lambdaCheat);
    observe({data: data[person_id], 
             link: Binomial({p: successRate, n: n_questions})})
    query.add("successRate" + person_id, successRate)
  })
...
dataLoc = 'https://raw.githubusercontent.com/mhtess/psych201s/master/data/cheat.txt'

cheatCounts  <- rowSums(read.table(dataLoc, header=F,sep=","))
