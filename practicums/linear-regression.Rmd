...
observed_data <- data.frame(x = c(1,2,3,4,5,6,7,8,9), y = c(T,T,T,F,T,F,F,F,F))
...
The estimated intercept gives us the log odds that 0 will be T. This can be easier to think about in terms of the *regular* odds, so we exponentiate the value. Since the intercept is very large, `exp(intercept)` will be massive, meaning the odds are exceedingly in favor of 0 being T (though note that 0 doesn't appear in our tiny dataset, so this is hypothetical)! 

The estimated slope gives us the change in log odds for a 1-unit change in x. Note that most of our posterior for the slope is negative, hence we expect to see a dramatic *decrease* in the odds of being T as x increases. 
...
Indeed, we see that while there is quite a bit of variability in our posterior predictive, most of the logistic curves are going from positive to negative very rapidly, and this is most often happening between x = 2 and x = 3, corresponding to the cutpoint we see in our data.
...
This gives qualitatively similar estimates, though our dataset is too small for `glm` techniques to give reliable estimates (our priors, however uninformative, help solve this problem for us above -- our slopes and intercepts are constrained to be within an interval)
