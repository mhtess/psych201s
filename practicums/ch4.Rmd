...
var model = function() {
  ...
  foreach(observed_data, function(row) {
...
Now, use a more realistic prior assumption for the mu_i means. Theoretically, IQ distributions should have a mean of 100, and a standard deviation of 15. Make this change in the script, and re-run the inference. How do the estimates of IQ given by the means change? Why?
...
var model = function() {
  ...
  //};
  var mus = {
    p1: sample(GaussianDrift({mu: 100, sigma: 15})),
    p2: sample(GaussianDrift({mu: 100, sigma: 15})),
    p3: sample(GaussianDrift({mu: 100, sigma: 15}))
  }
...
resUnif <- runModelMCMC(IQmodelUniformPrior, dataToUse, numSamples = 100000)
resGaussian <- runModelMCMC(IQmodelGaussianPrior, dataToUse, numSamples = 100000)
