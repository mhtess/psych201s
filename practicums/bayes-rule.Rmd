...
We thus abstract the inference procedure, and using `inference_opts` in the `webppl` call to tell it to do Rejection Sampling with 10000 samples. We also tell it to Rejection sampling on `model`. 

We will use the `observe` function going forward.
